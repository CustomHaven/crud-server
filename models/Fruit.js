...
class Fruit {
  ...
  static create(data) {
    const newFruit = data;
    const fruit = Fruit.show(newFruit.name.toLowerCase());
  ...
  update(data) {
      const updatedFruit = Fruit.show(this.name.toLowerCase());
      if (updatedFruit) {
  ...
  destroy() {
    const deletedFruit = Fruit.show(this.name.toLowerCase());
